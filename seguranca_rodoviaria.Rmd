...
Os dados de vários ficheiros foram importados e unidos numa unica tabela: `carspeed`.   
...
Velocidade média em ambas as direções:

```{r, echo=FALSE }

VelocidadeMédia<- c(round(mean(filter(df, Direção == 'MemMartins-Sintra')$Velocidade)),
  round(mean(filter(df, Direção == 'Sintra-MemMartins')$Velocidade)))

Direção <- c("Mem Martins - Sintra", "Sintra - Mem Martins")

knitr::kable(tibble(Direção, VelocidadeMédia))
```

Outra visualização interativa dos dados obtidos onde se podem verificar os dados com detalhe ao dia, hora ou mês:

```{r }

myChart <- gvisAnnotationChart(df, "Data", "Velocidade", 
                         idvar = "Direção", 
